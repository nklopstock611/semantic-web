...
def verify_unique_papers(file_path: str='/workspaces/semantic-web/first-task/pdf-downloader/publication_dates_2.json'):
    """
...
def create_json(metadata: dict) -> None:
    for each_article in metadata:
        with open('/workspaces/semantic-web/first-task/metadata3.json', 'r') as f:
            json_dict = json.load(f)

        json_dict.update({ each_article: metadata[each_article] })

        with open('/workspaces/semantic-web/first-task/metadata3.json', 'w') as f:
            json.dump(json_dict, f)
